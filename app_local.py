  ...
  <form id="genForm">
    ...
    <div class="tab-panel" id="panel-sitemap" style="display:none">
      <input type="url" id="sitemap_url" name="sitemap_url"
             ...
             style="width:100%;padding:12px 14px;border:1.5px solid #e0e0e0;border-radius:10px;font-size:14px;outline:none;margin-bottom:20px;">
    </div>
...
<script>
  ...
  document.addEventListener('change', function(e) {
    if (e.target.id === 'sitemap_file') {
      document.getElementById('fileNameXml').textContent = e.target.files[0]?.name ? '✓ ' + e.target.files[0].name : ''
    }
  })
